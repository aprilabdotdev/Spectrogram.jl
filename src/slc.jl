...
export SarImage, read_cdata, range_spectrogram, azimuth_spectrogram

Base.@kwdef struct SarImage  # keyword argument
    range
    azimuth
    data
end

function read_cdata(sar_image::SarImage)::Matrix{ComplexF32}
    stream = open(sar_image.data, "r+")   # default is read-only
